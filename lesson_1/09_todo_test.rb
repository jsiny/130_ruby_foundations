...
class TodoListTest < MiniTest::Test
  ...
  def test_add_wrong_todo
    assert_raises(TypeError) { @list.add(1) }
  end

  def test_add
    new_todo = Todo.new("Do stuff")
    @todos << new_todo
    @list << new_todo
    assert_equal(@todos, @list.to_a)
  end
